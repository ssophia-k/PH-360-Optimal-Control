...
class QLearning(Agent):
    ...
    def train(self, env : gym.wrappers, num_episodes : int=500) -> None:
        ...
        for episode in tqdm(range(num_episodes)):
            ...
            while not done:
                # 1. Select action with e-greedy policy
                ...
                next_t   = None if done else torch.tensor(next_state, dtype=torch.float32).unsqueeze(0)
                reward_t = torch.tensor([reward], dtype=torch.float32)

                self.buffer.append(Transition(state_t, action, reward_t, next_t))

                state = next_state

                # 4. Only train once we have enough samples in the buffer
                if len(self.buffer) <= self.batch_size:
                    continue

                # 5. Sample a random mini-batch and compute targets
                states, actions, targets = self.sample_buffer()

                # 6. Compute predicted Q-values for the taken actions
                predictions = self.policy_network(states).gather(1, actions)

                # 7. Gradient step to minimize MSE between predictions and targets
