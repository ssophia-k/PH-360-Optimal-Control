...
class ReinforceAgent(Agent):
    ...
    def learn(self, rewards: list, log_probs: list) -> None:
        ...
        # 1) Naive REINFORCE
        # R_tau = sum(self.gamma**t * rewards[t] for t in range(T))
        # log_probs_t = torch.stack(log_probs)
        # loss = -torch.sum(log_probs_t * R_tau)

        # 2) REINFORCE with causality trick
        # rewards_to_go = torch.tensor(
        #     [sum(self.gamma**(tp-t) * rewards[tp] for tp in range(t, T)) for t in range(T)],
        #     dtype=torch.float32
        # )
        # log_probs_t = torch.stack(log_probs)
        # loss = -torch.sum(log_probs_t * rewards_to_go)

        # 3) REINFORCE with causality trick + baseline
        rewards_to_go = torch.tensor(
            [sum(self.gamma**(tp-t) * rewards[tp] for tp in range(t, T)) for t in range(T)],
            dtype=torch.float32
        )
        ...
        log_probs_t = torch.stack(log_probs)
        loss = -torch.sum(log_probs_t * (rewards_to_go - baseline))
